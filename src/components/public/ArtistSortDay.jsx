...
export default function ArtistSortDay() {
    ...
    const [artistes, setArtistes] = useState([r])
    ...
    //cest bon 
    const artistesSortDay = artistes.reduce((acc, x) => {
        if(!acc[x.date]){
            acc[x.date] = []
        }
        acc[x.date].push(x)
    ...
    return (
        <div className="d-flex flex-column gap-5 widthCalendar fw-bolder noir p-0">
            <button onClick={()=>console.log(artistes)}>test</button>
            <button onClick={()=>console.log(artistesSortDay)}>test</button>
            ...
            <div className="d-flex flex-column">
                ...
                <div className="d-flex flex-column container-fluid">
                    ...
                    {/* {program.jeudi.map((artiste, index) => (
                        <div key={index} onClick={() => handleNavigate(`/programmation/${artiste.name}`, { state: { artiste } })} className="clickable d-flex justify-content-around  bgBlanc py-3 rounded text-center gap-1 gap-md-3 gap-lg-5 row px-3 border">
                            <p className="m-0 my-auto p-0 col-3">{artiste.name}</p>
                            <p className="m-0 my-auto p-0 col-3">{artiste.time}</p>
                            <p className="m-0 my-auto p-0 col-3">{artiste.stage}</p>
                        </div>
            ...
            <div className="d-flex flex-column">
                ...
                <div className="d-flex flex-column container-fluid">
                    ...
                    {/* {program.samedi.map((artiste, index) => (
                        <div key={index} onClick={() => handleNavigate(`/programmation/${artiste.name}`, { state: { artiste } })} className="clickable d-flex justify-content-around  bgBlanc py-3 rounded text-center gap-1 gap-md-3 gap-lg-5 row px-4 border">
                            <p className="m-0 my-auto p-0 col-3">{artiste.name}</p>
                            <p className="m-0 my-auto p-0 col-3">{artiste.time}</p>
                            <p className="m-0 my-auto p-0 col-3">{artiste.stage}</p>
                        </div>
